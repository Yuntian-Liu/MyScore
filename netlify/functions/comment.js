...
export default async (req) => {
  ...
  try {
    ...
    const apiBaseUrl = Netlify.env.get("AI_BASE_URL") || "https://api.deepseek.com";
    ...
    if (!response.ok) {
      ...
      return jsonResponse({ error: upstreamMessage }, response.status);
    }

    const aiComment =
      data?.choices?.[0]?.message?.content || "我在这，先深呼吸一下。";

    return jsonResponse({ comment: aiComment });
  } catch (error) {
