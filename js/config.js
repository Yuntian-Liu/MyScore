...
export const APP_VERSION = '5.3.0-beta';
...
<div class="changelog-entry">
  ...
  <div class="changelog-section">
    ...
    <ul class="changelog-list">
      <li><strong>游戏化 UI 组件</strong>：导航栏经验条（等级+进度+XP 数字）、连续打卡胶囊（⚡/🔥+天数）、成就墙（已解锁/未解锁状态，N/12 计数）</li>
      <li><strong>个人名片</strong>：点击头像查看名片，包含 Hero Banner、等级卡片（shimmer 动画）、数据概览三列、成就墙 3×4 网格、经验来源明细</li>
      <li><strong>Profile Panel 增强</strong>：头像下拉面板新增 XP 经验条、连续打卡摘要、成就摘要、"查看名片"入口</li>
      <li><strong>AI 流式输出</strong>：后端 SSE 流式代理 + 前端逐字渲染，AI 评论和伴学助手均支持，自动 fallback 非流式</li>
      <li><strong>AI 流式输出 UX</strong>：「正在思考…」动态省略号 + 打字机闪烁光标动效</li>
      <li><strong>Dashboard UI 重构</strong>：最近成绩摘要卡片、考试类型概览网格化、Sparkline 迷你趋势线、Hero 归档区</li>
      <li><strong>Slide Panel 二级面板</strong>：考试类型详情、最近成绩详情、历史记录（右侧滑入，移动端底部抽屉）</li>
      <li><strong>导入导出按钮整合</strong>：导出报告/导出数据/导入备份整合至考试概览区右上角</li>
      <li><strong>日志导出增强</strong>：新增登录状态、SW 缓存状态、PWA 安装状态、成就详情等诊断信息</li>
    </ul>
  </div>
  <div class="changelog-section">
    <div class="changelog-section-title">
    ...
    <ul class="changelog-list">
      <li>Slide Panel 移动端适配 — 全宽底部抽屉模式，修复内容偏窄和左侧溢出</li>
      <li>考试类型卡片 —「查看详情」按钮固定右下角，有无折线图都对齐</li>
      <li>Footer 简化 — 删除 matrix 网格，改为 pill 链接</li>
      <li>伴学助手流式输出 — 突突er 支持流式逐字显示，体验更自然</li>
      <li>使用文档更新 — 补充个人名片、流式输出、游戏化 UI 说明</li>
    </ul>
  </div>
  <div class="changelog-section">
    <div class="changelog-section-title">
    ...
    <ul class="changelog-list">
      <li>SW 缓存未更新导致服务器 UI 崩溃 — Logo 渐变残留、XP 条/打卡/成绩数据异常</li>
      <li>个人名片 XP 显示负数 — 用取模替代循环减法计算等级内经验</li>
    </ul>
  </div>
</div>`;
...
export const BETA_BANNER = {
    enabled: true,
    items: [
        'V5.3.0-beta 更新：趋势图各科预测、报告导出 html2canvas、SVG 雷达图、成就系统修复',
        '点击头像查看个人名片，展示等级/经验/成就/打卡',
        'AI 评论流式逐字输出 + 「正在思考」打字机动效',
