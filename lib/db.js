...
// 内存中的 UID 计数器，避免并发读取文件导致重复
let uidCounter = null;

export function getNextUid() {
    if (uidCounter === null) {
        const data = readJsonFile(UID_COUNTER_FILE, { next: 1100000 });
        uidCounter = data.next;
    }
    const uid = uidCounter;
    uidCounter++;
    writeJsonFile(UID_COUNTER_FILE, { next: uidCounter });
    return uid;
}
...
export function findUserById(id) {
    ...
    for (const email in users) {
        if (users[email].id === idNum) return users[email];
    }
    return null;
}
...
export function registerUser(email, nickname, avatarSeed, bio, passwordHash, salt, isBeta) {
    const users = readJsonFile(USERS_FILE, {});
...
export function updateUserFeishuOpenId(email, openId) {
    const users = readJsonFile(USERS_FILE, {});
    const user = users[email];
    if (!user) return null;
...
export function findUserByFeishuOpenId(openId) {
    const users = readJsonFile(USERS_FILE, {});
    for (const email in users) {
        if (users[email].feishu_open_id === openId) return users[email];
    }
    return null;
}
