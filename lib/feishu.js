...
export function buildLevelCard(xpData) {
  ...
  const xpForNext = 80 * level;
  const xpInLevel = total - (80 * (level - 1) * level / 2);
  ...
  return {
    ...
    elements: [
      ...
      { tag: "hr" },
      { tag: "markdown", content: "距下一级　" + xpInLevel + " / " + xpForNext + " XP　**" + pct + "%**\n`" + bar + "`" }
    ]
  };
}
...
async function handleLevelCommand(openId) {
  ...
  console.log("[Feishu] Level command:", "userId=", user.id, "xp_data=", JSON.stringify(xpData));

  const card = buildLevelCard(xpData);
  console.log("[Feishu] Level card:", JSON.stringify(card));
  const result = await sendInteractiveCard(openId, card);
  console.log("[Feishu] Level send result:", JSON.stringify(result));
}
